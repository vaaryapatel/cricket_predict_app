...
if predict_btn:
    if not model_loaded:
        ...
        for cls, prob in zip(model_ml.classes_, pred_proba):
            ...
            lbl_color = c["color"] if is_top else "rgba(232,245,236,0.4)"
            val_color = c["color"] if is_top else "rgba(232,245,236,0.5)"
            bar_bg    = c["bar"]
            prob_rows += (
                '<div class="prob-row">'
                f'<div class="prob-label" style="color:{lbl_color};">{cls}</div>'
                '<div class="prob-track">'
                f'<div class="prob-fill" style="width:{pct:.1f}%; background:{bar_bg};"></div>'
                '</div>'
                f'<div class="prob-val" style="color:{val_color};">{pct:.1f}%</div>'
                '</div>'
            )
